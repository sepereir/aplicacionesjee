...
public class Transaccion {
    ...
    public Transaccion(int id, int monto, Date fecha, String tipo, int idCategoria, int idCuenta) {
        super();
        this.id = id;
        this.monto = monto;
        this.fecha = fecha;
    ...
    public boolean getTransaccion(int x) {
        ...
        try {
            rs =
 st.executeQuery("SELECT * FROM" + " (TRANSACCION AS T JOIN CATEGORIA AS CAT ON CAT.idCategoria = T.Categoria_idCategoria)" +
                 " JOIN CUENTA AS C ON C.idCuenta = T.Cuenta_idCuenta" + " WHERE T.idTransaccion = " + x);
        ...
        try {
            if (rs.next()) {
                ...
                if (rs2.next())
                    setFecha_limite(rs2.getDate("fecha_limite"));
                flag = true;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public boolean borrarTransaccion(){
        // "Implementar"
        return true;
        //
        //
    ...
    public boolean crearTransaccion() {
        ...
        try {
            String insertcmd =
                "INSERT INTO TRANSACCION(idTransaccion, monto, fecha, tipo, Categoria_idCategoria, Cuenta_idCuenta)" +
                "VALUES(Transaccion_idTransaccion_SEQ.NEXTVAL, " + getMonto() + ", ?, " + "'" + getTipo() + "', " +
                getIdCategoria() + ", " + getIdCuenta() + ")";
    ...
    static public boolean crearTransaccionInterna(Transaccion t1, Transaccion t2) {
        ...
        try {
            st = con.prepareStatement("INSERT INTO TRANSACCION(monto, fecha, tipo, Categoria_idCategoria, Cuenta_idCuenta) VALUES(" +
                      " " + t1.getMonto() + "," + " '" + t1.getFecha() + "'," + " '" + t1.getTipo() + "'," + " " +
                      t1.getIdCategoria() + "," + " " + t1.getIdCuenta() + ")", Statement.RETURN_GENERATED_KEYS);
            st.executeUpdate();
            ...
            if (t1.getTipo() == "PRESTAMO" && t1.getFecha_limite() != null && rs.next()) {
                st =
 con.prepareStatement("INSERT INTO PRESTAMO(fecha_limite, Transaccion_idTransaccion) VALUES( " + " '" +
                      t1.getFecha_limite() + "'," + " " + id1 + ")");
                st.executeUpdate();
            }

            st =
 con.prepareStatement("INSERT INTO TRANSACCION(monto, fecha, tipo, Categoria_idCategoria, Cuenta_idCuenta) VALUES(" +
                      " " + t2.getMonto() + "," + " '" + t2.getFecha() + "'," + " '" + t2.getTipo() + "'," + " " +
                      t2.getIdCategoria() + "," + " " + t2.getIdCuenta() + ")", Statement.RETURN_GENERATED_KEYS);
            st.executeUpdate();
            rs = st.getGeneratedKeys();
            id2 = rs.getInt(1);
            rs.close();
 ...
 con.prepareStatement("INSERT INTO TRANSACCION_INTERNA(Transaccion_idTransaccion, Cuenta_idCuenta) VALUES(" + " " +
            ...
            rs =
 st.executeQuery("SELECT tipo, sum(monto) AS suma FROM TRANSACCION WHERE idCuenta = " + t1.getIdCuenta() +
            ...
            rs =
 st.executeQuery("SELECT tipo, sum(monto) AS suma FROM TRANSACCION WHERE idCuenta = " + t2.getIdCuenta() +
                 " GROUP BY tipo ORDER BY tipo");
            if (rs.next()) {
                int gasto = rs.getInt("suma");
                rs.next();
